...
SELECT T.DangNhapId,U.full_name,U.email,T.LoginTime FROM THONGKEDANGNHAP as T inner join user_info as U on T.id=U.id;
SELECT U.full_name, U.email, COUNT(T.DangNhapId) AS login_count, MIN(T.LoginTime) AS login_day
FROM THONGKEDANGNHAP AS T
INNER JOIN user_info AS U ON T.id = U.id
WHERE T.LoginTime = '2024-04-19'
GROUP BY U.full_name, U.email;

SELECT T.TruyVanId,T.id,U.full_name,T.TruyVanText,T.TraLoiText,T.ThoiGian FROM THONGKETRUYVAN as T inner join user_info as U on T.id=U.id;
select * from MONHOC;
